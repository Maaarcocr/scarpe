...
module Scarpe
  ...
  class App
    ...
    # Bind Ruby code to an automatically-generated JS function name, then call the JS function
    def ruby_eval(&code)
      raise "Cannot ruby_eval on closed or inactive Scarpe::App!" unless @w
      puts "RUBY EVAL: #{code.inspect}" if @opts[:debug]
      func_name = "eval#{code.object_id}"
      @w.bind(func_name, &code)
      @w.eval(func_name + "();")
    end
  ...
  class DebugWrapper
    ...
    def method_missing(*args, &block)
      puts "Debug: calling #{args.inspect}"
      @internal.send(*args, &block)
    end
  end
end
